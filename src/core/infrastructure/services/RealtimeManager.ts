...
export class RealtimeManager {
  ...
  public static getInstance(config?: RealtimeManagerConfig): RealtimeManager {
    if (!RealtimeManager.instance) {
      const defaultConfig: RealtimeManagerConfig = {
        ...
        maxJoinsPerSecond: 100,         // Aumentado para padrão oficial
        healthCheckInterval: 60000,     // Aumentado para 60s (menos agressivo)
        maxReconnectAttempts: 3,        // Reduzido para evitar loops
        reconnectBackoffMs: 5000       // Aumentado para 5s backoff
      };
  ...
  public unsubscribe(channelId: string): void {
    console.log(`[RealtimeManager] 🔌 Unsubscribe solicitado: ${channelId}`);
  ...
  private createChannel(subscription: ChannelSubscription): void {
    ...
    subscription.tables.forEach(table => {
      ...
      console.log(`[RealtimeManager] 👂 Configurando listener para tabela: ${table}${filter ? ` (filtro: ${filter})` : ''}`);
    ...
    // 🎯 CONFIGURAR STATUS HANDLERS
    channel.on('system', {}, (status) => {
      console.log(`[RealtimeManager] 🔧 Status do sistema (${subscription.channelId}):`, status);
      
      if (status.error) {
        this.handleChannelError(subscription.channelId, status.error);
      }
    });
